...
;; 1) "talker" >> "believer"
;; 2) "silent" >> "doubter"
;; num-food-strat is set at 3 (regular, super and poison food)
;; all agent always communicate
;; added "super" SLIDER in Interface - this is the energy level provided by super food
;; added "poison" SLIDER in Interface - this is the energy level deducted by poison food
;; colour of agents were changed
;; plot-all reconfigured


;;;;;;;;;;;;;;;;;;;;;;
;; PARAMETERS
;; Energy provided : super / poison
;; energy-required-to-reproduce
;; lifespan
;; decrease-life (not used)
;;
...
;;;;;;CHANGED;;;;;;
breed [ believer a-believer ] ;;was talker
breed [ doubter a-doubter ] ;;was silencer
...
to setup-globals
  ...
  set expected-graph-max 4000        ;; hard coded from looking at graphs
  ...
  set tc 25                 ;; color for ignorant turtles when using the "show knowledge" buttons
  set ktc 45               ;; color for turtles who know what you want to check on, as per previous line
...
;;;;;CHANGED COLOR;;;;;;
to update-patches
  if (here-list = [0 0 0] ) [set pcolor 0] ;;empty patch
  if (here-list = [1 0 0] ) [set pcolor 83] ;;regular food
  if (here-list = [0 1 0] ) [set pcolor 65] ;;super food alone
  if (here-list = [0 0 1] ) [set pcolor 123] ;; poison alone
 ;; if (here-list = [1 1 0] ) [set pcolor 65] ;;regular food + super food

  ;;if (here-list = [0 1 1] ) [set pcolor 5] ;; neutralised by poison and super food
  ;;if (here-list = [1 0 1] ) [set pcolor 15] ;; poison and regular food (low level poison)
  ;;if (here-list = [1 1 1] ) [set pcolor 82] ;; regular and poison and super food = regular


  ;;if pxcor > 0 [set pcolor (40 + (first here-list * 2) + (sum (butfirst here-list) * 6))]
 ;; if pxcor <= 0 [set pcolor (80 + (first here-list * 2) + (sum (butfirst here-list) * 6))]
end
...
to setup-agents
  set-default-shape believer "loud" ;;loud
  set-default-shape doubter "silent" ;;silent
...
to init-soc-vars
  ;; roll dice for breed
  ifelse ((random 100) < (starting-proportion-believers * 100))  [ set breed believer ] [ set breed doubter ]
  mutate-or-not
...
to mutate-or-not
    if (mutation?) [if (1 = (random (10 ^ freq-of-mutation))) [ifelse (breed = believer) [ set breed doubter ] [ set breed believer ]]]
end
...
to go
  ...
  ask (turtles) [
                take-food
                if (random energy) > energy-needed-to-reproduce [give-birth ]   ; "30" should really be a variable too; this is bad style to bury a parameter like this. Determines amount of investment per child
                if (show-knowledge != 0)
...
; here-list is taken to refer to a value of patch-here. This is good
   ...
   if ((item 0 knowhow = 1) and (item 1 here-list = 1))  ;check for the super food 1
     [set energy (energy + super)
     ]

   if ((item 1 knowhow = 1) and (item 2 here-list = 1))  ;check for the poisonous food 2
     [set energy (energy + poison) ; reduce energy
     set age (age + decrease-life) ; increase age, essentially decrease life, but this is not used in this study
     ]
  ...
  if (num-special-food-strat != 0) [
    ...
    if (sum knowhow) > 0
      [set n (one-of (non-zero knowhow 0))
        ;;;;CHANGED "turtle" to "believer"
       ask believer in-radius broadcast-radius ;; communication is only listened by believer, which means circulated knowledge is only acquired by believer
...
let ixi 0
  ...
  ;; (num-special-food-strat > 0) and (random-float 1 < p-knowhow) and
;;                        if ( xcor > 0) [
;;    set knowhow n-values num-special-food-strat [0]
;;                         [
;;                          set ixi (random 1 * 2)
...
to show-values
let soc-turtles (turtle-set believer doubter)
let n 0
  let t 0
...
if (any? believer)  [
   print (word "avarage knowhow believer: " (precision (mean ([sum knowhow] of believer)) 2) )
   print (word "standard deviation: " (precision (standard-deviation ([sum knowhow] of believer)) 2) ) ]
if (any? doubter)  [
   print (word "avarage knowhow doubter: " (precision (mean ([sum knowhow] of doubter)) 2) )
   print (word "standard deviation: " (precision (standard-deviation ([sum knowhow] of doubter)) 2) ) ]
end
...
set k ((mean ([sum knowhow] of believer)) / num-special-food-strat)
ask n-of c believer [die] ;kill off half the believers
create-doubter c                                      ;; create given number of silents
ask doubter [set age random lifespan                    ;; set age to hatchlings (fput kind hatchlingsrandom number < lifespan
            setxy (random world-width)  ;; randomize the turtle locations
                  (random world-height)
            set energy (random-normal 18 0.9 )
...
;;;;CHANGED;;;;
; plot-all is completely reconfigured
set-current-plot-pen  "ratio believer/doubter" ;; plot ratio of (all believer)/ (all turtles), ratio of believer / doubter
  plot ((count believer)/((count doubter) + (count believer)))

set-current-plot-pen "ratio believer-super / believer" ;; plot ratio of (believer who only eats regular food and super food) / (all believer)
  plot (count (believer with [(item 0 knowhow = 1) and (item 1 knowhow = 0)])) / (count believer)

set-current-plot-pen "ratio believer-poison / believer" ;; plot ratio of (believer who only eats regular food and poison food) / (all believer)
  plot (count (believer with [(item 1 knowhow = 1) and (item 0 knowhow = 0)])) / (count believer)

set-current-plot-pen "ratio believer-2-knowhow / believer" ;; plot ratio of (believer who eats regular food, super and poison food) / (all believer)
  plot (count (believer with [(item 0 knowhow = 1) and (item 1 knowhow = 1)])) / (count believer)

;;set-current-plot-pen "believer count" ;; plot believer count
;;  plot (count believer)
...
; plot ceiling (expected-graph-max * ((count believer) / ((count doubter) + (count believer))))
...
to-report energy-believers
report mean ([energy] of believer)
end

to-report energy-doubter
report mean ([energy] of doubter)
end
...
to-report avg-doubter-k [iii]
    report safe-mean [energy] of (doubter with [iii = sum knowhow])
end
to-report sd-doubter-k [iii]
    report safe-standard-deviation [energy] of (doubter with [iii = sum knowhow])
end
to-report count-doubter-k [iii]
    report count doubter with [iii = sum knowhow]
end
to-report avg-believer-k [iii]
    report safe-mean [energy] of (believer with [iii = sum knowhow])
end
to-report sd-believer-k [iii]
    report safe-standard-deviation [energy] of (believer with [iii = sum knowhow])
end
to-report count-believer-k [iii]
    report count believer with [iii = sum knowhow]
end
...
set-current-plot "t-s-knowhow"
set-current-plot-pen "doubter"
set-plot-pen-mode 1
histogram [sum knowhow] of doubter
set-current-plot-pen "believer"
set-plot-pen-mode 1
histogram [sum knowhow] of believer
end
...
while [iii < num-food-strat] [
  set-current-plot-pen "doubter"
  ifelse (any? doubter with [iii = sum knowhow])
    [plotxy iii mean [energy] of (doubter with [iii = sum knowhow])]
    [plotxy iii 0]
  set-current-plot-pen "believer"
  ifelse (any? believer with [iii = sum knowhow])
    [plotxy iii mean [energy] of (believer with [iii = sum knowhow])]
    [plotxy iii 0]
  set iii iii + 1
  ]
end
...
ifelse (any? believer with [sum-know-how = sum knowhow])
      [set ttt mean [energy] of (believer with [sum-know-how = sum knowhow])]
      [set ttt 0]
ifelse (any? doubter with [sum-know-how = sum knowhow])
      [set sss mean [energy] of (doubter with [sum-know-how = sum knowhow])]
      [set sss 0]
 report sss - ttt
end
@#$#@#$#@
...
"" ""
PENS
"ratio believer/doubter" 1.0 0 -7500403 true "" ""
"ratio believer-super / believer" 1.0 0 -13345367 true "" ""
"ratio believer-poison / believer" 1.0 0 -1184463 true "" ""
"ratio believer-2-knowhow / believer" 1.0 0 -15302303 true "" ""
...
323
331
ratio believer/doubter
(count believer)/((count doubter) + (count believer))
3
1
11
...
"" ""
PENS
"doubter" 1.0 1 -13345367 true "" ""
"believer" 1.0 1 -5825686 true "" ""
...
542
569
count believer-super
(count (believer with [(item 0 knowhow = 1) and (item 1 knowhow = 0)]))
17
1
11
...
682
570
count believer-poison
(count (believer with [(item 0 knowhow = 0) and (item 1 knowhow = 1)]))
17
1
11
...
885
569
ratio believer-super / believer
(count (believer with [(item 0 knowhow = 1) and (item 1 knowhow = 0)])) / (count believer)
3
1
11
...
598
624
count believer with 2 knowhow
(count (believer with [(item 0 knowhow = 1) and (item 1 knowhow = 1)]))
17
1
11
...
872
621
ratio believer-poison / believer
(count (believer with [(item 1 knowhow = 1) and (item 0 knowhow = 0)])) / (count believer)
3
1
11
...
900
674
ratio believer-2-knowhow / believer
(count (believer with [(item 0 knowhow = 1) and (item 1 knowhow = 1)])) / (count believer)
3
1
11
...
@#$#@#$#@
Authors: whet20
Commenting on changes from the original version by Cace and Bryson is also available in the code.


## WHAT IS IT?
This model is based on Cace & Bryson 2007 model (which showed that altruistic communication is adaptive). In this model, we showed that open-minded is adaptive. This model looks at the reasons why.
...
click Set Up
then GO
Feel free to change parameters on the right hand side e.g. energy-to-produce and super
...
+ ratio believer / doubter is fixed at 0.5
+ energy-needed-to-reproduce: vary energy needed to reproduce
+ super: energy provided to agent when super food is eaten
++ poison: energy provided to agent when super food is eaten (its negative number denoting energy reduction)
+ lifespan: life limit
+ decrease-life: age added when poison food is eaten (not used in this study) 

## DISPLAY:

+ plot-all: shows various ratio over time
+

Important counter
- ratio believer/doubter
- count believer-super (believer only eats regular and super food)
- counter believer-poison (believer only eats regular and poison)
- counter believer with 2 knowhow (believers who eat all food types)
- count doubter
- count all believer 

The below displays were not used:
...
Black [000] - empty
Purple [0 0 1] - poison
Green [0 1 0] - good food
Dark green [1 0 0] - normal food 
